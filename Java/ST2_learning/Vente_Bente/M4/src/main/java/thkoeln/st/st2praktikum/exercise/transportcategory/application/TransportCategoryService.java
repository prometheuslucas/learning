...
import thkoeln.st.st2praktikum.exercise.transportcatpackage thkoeln.st.st2praktikum.exercise.transportcategory.application;
...
public class TransportCategoryService {
    ...
    public UUID addConnection(UUID transportCategoryId,
        ...
        Connection newConnection = new Connection(sourceVector2D, sourceRoom, destinationVector2D, destinationRoom);

        transportCategory.getConnections().add(newConnection);
        return transportCategoryRepository.save(transportCategory).getId();
    }

    public Optional<Connection> getConnection(Room sourceRoom, Vector2D sourcePosition, UUID destinationRoomId) {
        Room destinationRoom = roomRepository.findById(destinationRoomId).get();
